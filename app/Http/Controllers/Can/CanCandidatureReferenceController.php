...
use App\Rules\Emploi\Candidature\CandidatureReferenceUniqueRule;
use Exception;
...
class CanCandidatureReferenceController extends Controller
{
    /**
     * Ajouter un referencelienCv à sa candidature.
     */
    public function lier(Request $requete): JsonResponse
    {
        try {
            ...
            $validateur = Validator::make($entrees, [
                ...
                'reference' => [
                    'required',
                    'exists:emploi_candidat_references,id',
                    new CandidatureReferenceUniqueRule,
                ],
            ]);
            ...
            // Récupérer le referencelienCv.
            $reference = CandidatCentre::find($entrees['reference']);

            if ($porte->denies('proprietaire-modele', $reference)) {
                throw new UnauthorizedException();
            }

            // Créer un lien referencelienCvs.
            $lien = CandidatureReference::create($entrees);
    ...
    /**
     * Supprimer un reference d'intérêts d'une candidature.
     */
    public function delier(Request $requete, string $id): JsonResponse
    {
        try {
            ...
            // Rechercher le lien.
            $lien = CandidatureReference::findOrFail($id);
